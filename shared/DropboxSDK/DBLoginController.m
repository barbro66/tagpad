...
- (void)viewDidLoad {
	...
	// - PKD
	initialSize = CGSizeMake(320, 420);
	if ([self respondsToSelector:@selector(setContentSizeForViewInPopover:)])
		self.contentSizeForViewInPopover = initialSize;
    ...
    tableView.scrollEnabled = YES; // since not using modal views - PKD
...
- (void)viewDidAppear:(BOOL)animated {
	...
	// To enable resizing within a popover - PKD
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad && popover.popoverContentSize.height > initialSize.height + 44)
		[NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(resizePopover) userInfo:nil repeats:NO];
}
- (void) resizePopover {
	[popover setPopoverContentSize:CGSizeMake(initialSize.width, initialSize.height + 44) animated:YES];	
}
@synthesize popover;
...
- (void)didPressCreateAccount {
	...
	createAccountController.popover = self.popover; // - PKD;
...
- (void)didPressCancel {
    ...
    // [self.navigationController.parentViewController dismissModalViewControllerAnimated:YES];  not using modal views for the login controller - PKD
...
- (void)restClientDidLogin:(DBRestClient*)client {
    [self setWorking:NO];
    // [self.parentViewController dismissModalViewControllerAnimated:YES]; not using modal views for the login controller - PKD
